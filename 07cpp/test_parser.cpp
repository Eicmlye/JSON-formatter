...
#define EXPECT_EQ_STDSTRING(expect, actual) \
    do {\
        char* str1 = new char[expect.size() + 1]; \
        char* str2 = new char[actual.size() + 1]; \
        strcpy(str1, expect.c_str()); \
        strcpy(str2, actual.c_str()); \
        EXPECT_EQ_STRING(str1, str2); \
        delete[] str1; \
        delete[] str2; \
    } while (0)
...
#define TEST_STRINGIFIER(vDirect, vIndirect, stdstr1, stdstr2, context, ifPrint) \
    do {\
        EXPECT_EQ_INT(Lept::PARSE_OK, vDirect.parse(context)); \
        EXPECT_EQ_INT(Lept::STRINGIFY_OK, vDirect.stringify(stdstr1)); \
        EXPECT_EQ_INT(Lept::PARSE_OK, vIndirect.parse(stdstr1)); \
        EXPECT_EQ_INT(Lept::STRINGIFY_OK, vIndirect.stringify(stdstr2)); \
        EXPECT_EQ_STDSTRING(stdstr1, stdstr2); \
        if (ifPrint)\
            std::cout << stdstr1 << std::endl << std::endl; \
    } while (0) /* use do-while to avoid reinitialization on stdstr */
#endif
...
static void testStringifier(void)
{
    Lept::Value v, w; 
    std::string JSONCache = {}, mem = {};
    //v.parse("{\"level\" : 1 , \"name\" : \"Eric\", \"ID\" : 10092, \"ally\" : [\"LOCK\", [1,2]], \"weapon\" : {\"mask\":\"N95\", \"dura\\tbility\" : 0.35}}"); 
    //v.parse("{\"level\":1, \"name\":\"\\ud834\\udd1e\"}");
    //v.parse("{\"name\":\"\\ud834\\udd1e\"}"); 
    //v.parse("{\"level\":1}");
    //v.parse("{\"a\":{\"b\":[1,2,[3,[4,5]], {\"c\":[]}]}}");

    bool printFlag = true;
    TEST_STRINGIFIER(v, w, JSONCache, mem, "null", printFlag);
    TEST_STRINGIFIER(v, w, JSONCache, mem, "false", printFlag);
    TEST_STRINGIFIER(v, w, JSONCache, mem, "true", printFlag);
    TEST_STRINGIFIER(v, w, JSONCache, mem, "0", printFlag);
    TEST_STRINGIFIER(v, w, JSONCache, mem, "-10", printFlag);
    TEST_STRINGIFIER(v, w, JSONCache, mem, "1.50", printFlag);
    TEST_STRINGIFIER(v, w, JSONCache, mem, "2.3e-04", printFlag);
    TEST_STRINGIFIER(v, w, JSONCache, mem, "\"Hello 1903.4, ?#$Eri\\nc\"", printFlag);
    TEST_STRINGIFIER(v, w, JSONCache, mem, "[]", printFlag);
    TEST_STRINGIFIER(v, w, JSONCache, mem, "[0.5, null, [true, \"yes Ma\'am\"]]", printFlag);
    TEST_STRINGIFIER(v, w, JSONCache, mem, "[1, [2, {\"3\": false}, 4.5], \"6\"]", printFlag);
    TEST_STRINGIFIER(v, w, JSONCache, mem, "{}", printFlag);

    TEST_STRINGIFIER(v, w, JSONCache, mem, "{\"a\":null, \"b\":true, \"c\":1.25, \"d\":[null, 3e04], \"e\":{\"inner\":\"Hello\"}}", printFlag);
    TEST_STRINGIFIER(v, w, JSONCache, mem, "{\"a\":{\"b\":[1,2,[3,[4,5]], {\"c\":[]}]}}", printFlag);
    TEST_STRINGIFIER(v, w, JSONCache, mem, "{\"level\" : 1 , \"name\" : \"Eric\", \"ID\" : 10092, \"ally\" : [\"LOCK\", [1,2]], \"weapon\" : {\"mask\":\"N95\", \"dura\\tbility\" : 0.35}}", printFlag);
...
int main(void) 
{
    ...
    printf("JSON parser: %d out of %d (%3.2f%%) tests passed. \n", test_pass, test_count, test_pass * 100.0 / test_count);

    RESET_TEST; 
    testStringifier();
    printf("JSON stringifier: %d out of %d (%3.2f%%) tests passed. \n", test_pass, test_count, test_pass * 100.0 / test_count);
